...
class _SliderItem {
  ...
  _SliderItem copyWith({double? ratio}) => _SliderItem(
    ...
    noteType: noteType, ratio: ratio ?? this.ratio, reason: reason,
  );
}
...
class _ClaudePanelState extends State<ClaudePanel> {
  ...
  Future<void> _restoreFromCache() async {
    ...
    setState(() {
      if (msgsStr != null) {
        final l = jsonDecode(msgsStr) as List;
        if (l.isNotEmpty) { _msgs.clear(); _msgs.addAll(l.cast<Map<String, dynamic>>()); }
      }
      if (recipeStr != null) {
        try { _recipe = jsonDecode(recipeStr); } catch (_) {}
      }
      if (slidersStr != null) {
        try {
          final l = jsonDecode(slidersStr) as List;
          _sliders = l.map<_SliderItem>((m) => _SliderItem(
            ingredientId:   m['ingredientId'] as int?,
            ingredientName: m['ingredientName']?.toString() ?? '',
            noteType:       m['noteType']?.toString() ?? 'top',
  ...
  void _persistMsgs() async {
    final p = await SharedPreferences.getInstance();
    await p.setString('studio_claude_messages',
        jsonEncode(_msgs.where((m) => m['isStatus'] != true).toList()));
  }
  void _persistRecipe() async {
    final p = await SharedPreferences.getInstance();
    if (_recipe != null) await p.setString('studio_claude_recipe', jsonEncode(_recipe));
    else await p.remove('studio_claude_recipe');
  }
  void _persistSliders() async {
    final p = await SharedPreferences.getInstance();
    await p.setString('studio_claude_sliders', jsonEncode(
        _sliders.map((s) => {
          'ingredientId': s.ingredientId, 'ingredientName': s.ingredientName,
          'noteType': s.noteType, 'ratio': s.ratio, 'reason': s.reason,
        }).toList()));
  }
  void _persistEval() async {
    final p = await SharedPreferences.getInstance();
    if (_eval != null) await p.setString('studio_claude_eval', _eval!);
    else await p.remove('studio_claude_eval');
  }
  ...
  void _addStatus(String content) {
    ...
    setState(() {
      ...
      _msgs.add({'role': 'assistant', 'content': content, 'isStatus': true, 'isEval': false});
    });
  }
  ...
  void _initSliders(Map<String, dynamic> parsed) {
    final items = <_SliderItem>[
      for (final n in (parsed['topNotes']    as List? ?? []))
        _SliderItem(ingredientId: n['ingredientId'] as int?,
          ingredientName: n['ingredientName']?.toString() ?? '', noteType: 'top',
          ratio: (n['ratio'] as num?)?.toDouble() ?? 0, reason: n['reason']?.toString()),
      for (final n in (parsed['middleNotes'] as List? ?? []))
        _SliderItem(ingredientId: n['ingredientId'] as int?,
          ingredientName: n['ingredientName']?.toString() ?? '', noteType: 'middle',
          ratio: (n['ratio'] as num?)?.toDouble() ?? 0, reason: n['reason']?.toString()),
      for (final n in (parsed['baseNotes']   as List? ?? []))
        _SliderItem(ingredientId: n['ingredientId'] as int?,
          ingredientName: n['ingredientName']?.toString() ?? '', noteType: 'base',
          ratio: (n['ratio'] as num?)?.toDouble() ?? 0, reason: n['reason']?.toString()),
    ];
    // ratio 정규화
    final total = items.fold<double>(0, (s, i) => s + i.ratio);
    if (total > 0 && (total - 1.0).abs() > 0.005) {
      for (final i in items) i.ratio = i.ratio / total;
    }
  ...
  void _handleSliderChange(int idx, double v) {
    ...
    _evalTimer = Timer(const Duration(seconds: 1), () {
      if (_prevSliders.isNotEmpty) _requestEval(_prevSliders, _sliders);
      _prevSliders = _sliders.map((s) => s.copyWith()).toList();
    });
  }
  ...
  Future<void> _requestEval(List<_SliderItem> prev, List<_SliderItem> curr) async {
    ...
    try {
      snap(List<_SliderItem> l, String t) => l.where((s) => s.noteType == t)
          .map((s) => {'ingredientName': s.ingredientName, 'ratio': s.ratio}).toList();
      toSnap(List<_SliderItem> l) => {
        'topNotes': snap(l, 'top'), 'middleNotes': snap(l, 'middle'), 'baseNotes': snap(l, 'base'),
      };
      ...
      if (res.statusCode == 200) {
        ...
        if (evalText.isNotEmpty && mounted) {
          setState(() {
            _eval = evalText;
            _msgs.add({'role': 'assistant', 'content': '🌿 조향 변화 평가\n$evalText',
  ...
  Future<void> _sendChat() async {
    ...
    try {
      ...
      String buffer = '', assistantContent = '';
        ...
        for (final line in lines) {
          ...
          try {
            ...
            if (data['status'] == 'extracting_keywords') {
              if (mounted) setState(() => _status = _Status.extracting);
              _addStatus('✦ Gemini가 향수 재료 키워드를 추출하고 있습니다...');

            } else if (data['status'] == 'searching_ingredients') {
              if (mounted) setState(() => _status = _Status.searching);
              _addStatus('✦ Supabase에서 매칭 재료를 검색하고 있습니다...');
              ...
              if (!assistantAdded && mounted) {
                setState(() {
                  _msgs.removeWhere((m) => m['isStatus'] == true);
                  _msgs.add({'role': 'assistant', 'content': '', 'isStatus': false, 'isEval': false});
                });
                assistantAdded = true;
                _scrollBottom();
              }
              ...
              if (rj != null && rj != '{}') {
                try {
                  final parsed = jsonDecode(rj) as Map<String, dynamic>;
                  if (mounted) setState(() => _recipe = parsed);
                  _initSliders(parsed);
                  _persistRecipe(); _persistSliders();
                } catch (_) {}
              }
              _persistMsgs();
      ...
      if (mounted) setState(() {
        ...
        _msgs.removeWhere((m) => m['isStatus'] == true);
        _msgs.add({'role': 'assistant', 'content': '오류가 발생했습니다. 다시 시도해 주세요.',
          'isStatus': false, 'isEval': false});
      });
      _persistMsgs();
    } finally {
  ...
  void _resetAll() {
    ...
    _evalTimer?.cancel();
    setState(() {
      _msgs.clear(); _initChat();
      _recipe = null; _sliders = []; _prevSliders = [];
      _eval = null; _status = _Status.idle; _inputCtrl.clear();
    });
  ...
  // ════════════════════════════════════════════════════════════
  // BUILD — 핵심: Column 전체를 Expanded로 감싸지 않고
  // 채팅 ListView만 Expanded로 처리
  // ════════════════════════════════════════════════════════════
  @override
    ...
    return Column(
      ...
      children: [
        ...
        if (_busy)
          Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Text(
                _status == _Status.extracting ? 'Gemini: 키워드 추출 중...'
                  : _status == _Status.searching ? 'Supabase: 재료 검색 중...'
                  : 'Claude: 조향 중...',
        ...
        Container(
          ...
          child: Row(children: [
            Expanded(
              child: Text('조향사와 대화하며 나만의 향수를 설계해보세요',
                  style: TextStyle(fontSize: 10,
                      color: _grey.withOpacity(0.8), fontStyle: FontStyle.italic)),
            ),
            GestureDetector(
              onTap: _resetAll,
              child: Container(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              final content  = (msg['content'] ?? '')
                  .replaceAll(RegExp(r'<recipe>[\s\S]*?</recipe>'), '');
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: isUser ? null : Border.all(
                              color: isEval ? _evalBorder : _cream),
                        ),
                          ...
                          Flexible(child: Text(content,
                              style: TextStyle(fontSize: 12, height: 1.6,
                                color: isUser ? Colors.white
                                  : isEval ? _evalGreen : _dark))),
                          if (_busy && isLast && !isUser && !isStatus)
                            Container(width: 2, height: 12, color: _gold,
          ...
          child: Row(children: [
            Expanded(
              child: Container(
                ...
                decoration: BoxDecoration(
                    color: _bg, border: Border.all(color: _cream)),
                child: TextField(
                  controller: _inputCtrl,
                  maxLines: null,
                  style: const TextStyle(fontSize: 13, color: _dark),
                  decoration: InputDecoration(
                    hintText: _busy ? '조향 중입니다...' : '향수 감성을 입력하세요...',
            ...
            const SizedBox(width: 8),
            GestureDetector(
              onTap: _busy ? null : _sendChat,
              child: Container(
                width: 40, height: 40,
                color: _busy ? _cream : _dark,
                child: _busy
                  ? const Center(child: SizedBox(width: 14, height: 14,
                      child: CircularProgressIndicator(color: _gold, strokeWidth: 1.5)))
                  : const Icon(Icons.send_rounded, color: _gold, size: 16),
              ),
            ),
          ]),
        ),
        ...
        Container(
          color: Colors.white,
          padding: const EdgeInsets.only(bottom: 6),
          child: const Text('Gemini → Supabase → Claude 3단계 파이프라인',
  ...
  Widget _buildRecipePanel() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: _dark, border: Border.all(color: _gold.withOpacity(0.3))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (_recipe!['concept'] != null)
                Text(_recipe!['concept'],
                    style: const TextStyle(fontSize: 9, color: _gold, fontStyle: FontStyle.italic)),
            ])),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 3),
              decoration: BoxDecoration(border: Border.all(
                  color: isBalanced ? _gold.withOpacity(0.4) : Colors.red.withOpacity(0.4))),
              child: Text('합계 ${(total * 100).round()}%',
                  style: TextStyle(fontSize: 9,
                      color: isBalanced ? _gold : Colors.redAccent)),
            ),
            const SizedBox(width: 8),
            GestureDetector(
              onTap: () {
                setState(() { _recipe = null; _sliders = []; _eval = null; });
                _persistRecipe(); _persistSliders(); _persistEval();
              },
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: ['top', 'middle', 'base'].expand((noteType) {
                final noteItems = _sliders.asMap().entries
                    .where((e) => e.value.noteType == noteType).toList();
                    ...
                    return Padding(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                          Flexible(child: Text(item.ingredientName,
                        ...
                        SliderTheme(
                          ...
                          child: Slider(
                            value: item.ratio, min: 0, max: 1, divisions: 100,
                            onChanged: (v) => _handleSliderChange(idx, v),
                          ),
                        ),
                        if (item.reason != null)
                          Text(item.reason!, style: TextStyle(fontSize: 8,
                              color: Colors.white.withOpacity(0.3), fontStyle: FontStyle.italic)),
                      ]),
                    );
                  }),
                ];
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('GEMINI EVALUATION',
                  style: TextStyle(fontSize: 8, letterSpacing: 3, color: _gold)),
              ...
              if (_evalLoading)
                const Row(children: [
                  ...
                  SizedBox(width: 6),
                  Text('평가 중...', style: TextStyle(fontSize: 9, color: _grey)),
                ])
              else if (_eval != null)
                Text(_eval!, style: const TextStyle(fontSize: 10, color: Colors.white60,
        ...
        Padding(
          ...
          child: Column(children: [
            if ([_recipe!['concentration'], _recipe!['recommendedSeason'],
                _recipe!['recommendedOccasion']].any((v) => v != null))
              Padding(
                padding: const EdgeInsets.only(bottom: 8),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: const Text('조향 저장하기',
                    style: TextStyle(fontSize: 10, letterSpacing: 3, fontWeight: FontWeight.w600)),
              ),
            ),
          ]),
        ),
  ...
  Widget _meta(String label, String value) => RichText(text: TextSpan(children: [
    TextSpan(text: '$label  ', style: const TextStyle(fontSize: 8, color: _gold)),
    TextSpan(text: value, style: const TextStyle(fontSize: 9, color: Colors.white60)),
  ]));
}