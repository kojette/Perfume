...
import { Plus, Edit2, Trash2, Gift, Coins, Check, X, Loader2 } from 'lucide-react';
...
const CouponPointManagement = () => {
  ...
  const handleDeleteCoupon = async (id) => {
    ...
    else { alert('삭제되었습니다.'); fetchCoupons(); }
  };

  const handleDeletePointRule = async (id) => {
    if (!window.confirm('정말 삭제하시겠습니까?')) return;
    const { error } = await supabase.from('PointRules').delete().eq('id', id);
    if (error) console.error('삭제 에러:', error);
    else { alert('삭제되었습니다.'); fetchPointRules(); }
  };
  ...
  const togglePointRule = async (id, currentStatus) => {
    ...
    else fetchPointRules();
  };

  const resetCouponForm = () => {
    setCouponData({
      code: '', discount_type: 'PERCENTAGE', discount_value: 0, min_purchase: 0,
      max_discount: null, expiry_date: '', usage_limit: 1, is_stackable: false
    });
  ...
  const resetPointForm = () => {
    setPointData({
      rule_name: '', action_type: 'PURCHASE', point_rate: 0.1, max_points: 100, is_active: true
    });
        ...
        <div className="text-center mb-12">
          ...
          <div className="flex justify-center gap-4 mb-8">
            <button
              ...
              className={`px-8 py-3 text-xs tracking-[0.3em] transition-all ${activeTab === 'coupons' ? 'bg-[#c9a961] text-white' : 'bg-white text-[#8b8278] border border-[#c9a961]/20 hover:border-[#c9a961]'}`}
            >쿠폰 관리</button>
            <button
              onClick={() => setActiveTab('points')}
              className={`px-8 py-3 text-xs tracking-[0.3em] transition-all ${activeTab === 'points' ? 'bg-[#c9a961] text-white' : 'bg-white text-[#8b8278] border border-[#c9a961]/20 hover:border-[#c9a961]'}`}
            >포인트 규칙</button>
          </div>
        </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setCouponData({...couponData, code: e.target.value.toUpperCase().replace(/\s/g, '')})}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                        <span className="px-4 py-1.5 bg-[#c9a961] text-white text-sm font-mono font-bold rounded">{coupon.code}</span>
                        ...
                        {coupon.is_stackable && <span className="px-2 py-1 bg-green-500 text-white text-[9px] rounded">중복 가능</span>}
                      </div>
                    ...
                    <button onClick={() => handleDeleteCoupon(coupon.id)} className="p-2 text-red-500 hover:bg-red-50 rounded transition-all">
                      <Trash2 size={18} />
                    </button>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-3 pt-4">
                    <button
                      ...
                      className="flex-1 py-3 bg-[#c9a961] text-white text-xs tracking-[0.3em] hover:bg-[#b89851] transition-all flex justify-center"
                    >
                      {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : '생성 완료'}
                    </button>
                    <button onClick={resetPointForm} className="flex-1 py-3 border border-[#c9a961] text-[#c9a961] text-xs tracking-[0.3em] hover:bg-[#faf8f3] transition-all">취소</button>
                  </div>
                </div>
              </div>
            )}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <span className={`px-2 py-1 text-[9px] rounded ${rule.is_active ? 'bg-green-500 text-white' : 'bg-gray-400 text-white'}`}>{rule.is_active ? '활성화' : '비활성화'}</span>
                      </div>
                      <div className="text-sm text-[#555] space-y-1">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => togglePointRule(rule.id, rule.is_active)} className={`p-2 rounded transition-all ${rule.is_active ? 'text-orange-500 hover:bg-orange-50' : 'text-green-500 hover:bg-green-50'}`}>
                        {rule.is_active ? <X size={18} /> : <Check size={18} />}
                      </button>
                      <button onClick={() => handleDeletePointRule(rule.id)} className="p-2 text-red-500 hover:bg-red-50 rounded transition-all">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </div>
